...
typedef struct sa_mqtt_instance_s {
    MQTTAsync *client;
    int qos;
    sa_circular_buffer *subscriber_buff;
    ohandle subscription_emit_sym;
    char name[20];
} sa_mqtt_instance;
...
int getopts(bindtype env, ohandle rec, struct pubsub_opts *opts)
{
    ...
    else if (opts->publisher == 1)
    {
        ...
        maybe_set_string(env, opts->user_property.value, rec, "user-property-value");
...
int messageArrived(void *context, char *topicName, int topicLen, MQTTAsync_message *message)
{
    ...
    sa_circular_buffer_push(sa_cxt->subscriber_buff,&item);
    return 1;
}
...
#define CLIENT_POP_BUFFER_SIZE 100
#define SA_MQTT_CIRC_BUFF_SIZE 1000
#define SA_MQTT_THREAD_SLEEP_TIME 1000 * 16

void client_thread(size_t arg)
{
    sa_thread_initialize();
    sa_connection c;
    sa_connect(&c,"");
    ...
    while(TRUE)
    {
        ...
        int pos;
        for(pos = 0; pos < CLIENT_POP_BUFFER_SIZE; pos++)
        {
            void *e = sa_circular_buffer_pop(context->subscriber_buff);
            if(e != NULL){
                temp_buff[pos] = e;
            } else {
                break;
        ...
        if(pos > 0) {
            int i;
            for(i = 0; i < pos && temp_buff[i] != NULL; i++) 
            {
                ohandle res = nil;
                ohandle mem = nil;
                ohandle topic = nil;
                ...
                strncat(full_topic,item->topicName,200);
                sa_makestring(&topic,full_topic);
                sa_makebinary(&mem,item->message->payload,item->message->payloadlen);
                sa_call_lisp(&res,c,context->subscription_emit_sym,NULL,4,topic,mem,nil,t);
                sa_free(&mem,&res,NULL);
        ...
        {
            // SLEEP!
            sa_sys_sleep(SA_MQTT_THREAD_SLEEP_TIME);
        }
    }
    sa_thread_finalize();
}
...
ohandle mqtt_publishfn(bindtype env,ohandle instance, ohandle topic, ohandle value)
{
    ...
    if(a_datatype(instance) != memorytype) return lerror(ILLEGAL_ARGUMENT,instance,env);
    ...
    pubmsg.retained = 0;
    i = 0;
    while ((rc = MQTTAsync_sendMessage(*context->client, dtopic, &(pubmsg), &(pub_opts))) != MQTTASYNC_SUCCESS )
    {
    ...
    if(a_datatype(value) == BINARYTYPE) free(pubmsg.payload);
    if(str != nil) a_free(str);
    return nil;
}
...
ohandle mqtt_subscribefn(bindtype env,ohandle instance, ohandle topic)
{
    ...
    while ((rc = MQTTAsync_subscribe(*(context->client), dtopic, context->qos, &ropts)) != MQTTASYNC_SUCCESS && i++ < 10)
    {
        a_sleep(0.1*i*i);
    }
    if(i == 10)
...
ohandle mqtt_unsubscribefn(bindtype env, ohandle instance, ohandle topic)
{
    ...
    ropts.context = (context->client);
    int rc;
    if ((rc = MQTTAsync_unsubscribe(*(context->client), dtopic, &ropts)) != MQTTASYNC_SUCCESS)
    {
        a_printerror(env, -1, MQTTAsync_strerror(rc), topic);
        return nil;
    }
...
ohandle mqtt_register_clientfn(bindtype env,ohandle name, ohandle opts_record)
{
    ...
    if(stringlen(name) > 20) {
        return a_printerror(env, -1, "Broker id cannot be more than 20 chars.", opts_record); 
    } else {
        char *temp;
    ...
    rc = MQTTAsync_createWithOptions(client, opts.connection, opts.clientid, MQTTCLIENT_PERSISTENCE_NONE,
                                     NULL, &createOpts);
    ...
    sa_sys_thread_begin(client_thread,(size_t)context);
    return map_memory(context,sizeof(context),free_mqtt_client);
}
