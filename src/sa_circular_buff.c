...
sa_circular_buffer *sa_make_circular_buffer(size_t size, size_t item_size)
{
    ...
    if(buff->buffer == NULL) return NULL;
    ...
    for(i = 0; i < buff->buffer_size; i++) {
        ...
        if(buff->buffer[i] == NULL) return NULL;
    }
    return buff;
}
...
{
    void *ret = NULL;
    if(!sa_circular_buffer_can_read(buff)) return ret;
...
{
    if(!sa_circular_buffer_can_write(buff)) return 0;
